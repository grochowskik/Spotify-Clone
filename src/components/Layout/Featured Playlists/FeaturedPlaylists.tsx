function FeaturedPlaylists({featuredPlaylists}) {
    ...
    return (
        ...
        <section className="grid text-cyan-50 col-start-2 col-span-5 grid-cols-4 mt-8">
          <p className='col-span-4 text-zinc-500 font-mono border-b-2 border-zinc-700 text-2xl'>Something for your taste</p>
          {featuredPlaylists.items.map(playlist => {
              return (
              <a href={'/playlist/' + playlist.id} key={playlist.id} className='mx-auto my-4 text-zinc-300 font-mono '>
                  <img src={playlist.images[0].url} width='300' alt="playlist cover"/>
