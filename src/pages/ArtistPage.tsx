...
function ArtistPage() {
  ...
  const artistAlbumsData = useArtistsAlbumsFetch(params.id!, {limit: '4', offset: '0'})
  const artistData = useArtistFetch(params.id!)

  if (artistAlbumsData.isLoading || artistTopSongsData.isLoading || artistData.isLoading) {return <h1>Loading</h1>}
  if (artistAlbumsData.isError || artistTopSongsData.isError || artistData.isError) {return <h1>Error</h1>}
